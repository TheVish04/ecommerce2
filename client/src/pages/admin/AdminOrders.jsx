...
const AdminOrders = () => {
    ...
    const getItemNames = (order) =>
        order.products?.map(p => `${p.product?.title || 'Unknown'} × ${p.quantity || 1}`).filter(Boolean).join(', ') || '-';
    ...
    return (
        <div className="space-y-6">
            ...
            <p className="text-gray-400 text-sm">View and update delivery status for all platform orders.</p>
                    ...
                    orders.map((order) => (
                        <div key={order._id} className="glass-card p-6">
                            ...
                            <div className="mt-4 pt-4 border-t border-white/10">
                                <p className="text-gray-300 text-sm">{getItemNames(order)}</p>
                                <p className="text-gray-500 text-xs mt-1">Customer: {order.buyer?.name} • {formatDate(order.createdAt)}</p>
                                <p className="text-emerald-400 font-mono font-bold mt-2">₹{order.totalAmount?.toLocaleString()}</p>
                            </div>
                        </div>
                    ))
                )}
