...
const VendorLayout = () => {
    ...
    return (
        <div className="flex h-screen bg-[#0f1014] text-white overflow-hidden">
            ...
            {/* Mobile Bottom Nav (Optional, handled via hidden md:flex above, need mobile solution or just desktop focus for now? User said responsive) */}
            {/* For simplicity in this step, I'll stick to desktop/tablet sidebar. Mobile users might need a hamburger menu or bottom nav. */}

            {/* Mobile Bottom Navigation */}
            <div className="md:hidden fixed bottom-0 left-0 right-0 bg-[#0f1014]/90 backdrop-blur-lg border-t border-white/5 z-50 px-6 py-3 flex justify-between items-center">
                {sidebarResult.map((item) => {
                    const isActive = location.pathname === item.path;
                    return (
                        <Link
                            key={item.path}
                            to={item.path}
                            className={`flex flex-col items-center gap-1 ${isActive ? 'text-blue-400' : 'text-gray-500'
                                }`}
                        >
                            <item.icon size={20} />
