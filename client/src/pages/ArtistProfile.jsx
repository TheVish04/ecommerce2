...
const ArtistProfile = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // Fetch Artist Info
                const artistRes = await axios.get(`http://localhost:3001/api/users/artist/${id}`);
                setArtist(artistRes.data);

                // Fetch Artist Products (Public)
                const productsRes = await axios.get(`http://localhost:3001/api/users/artist/${id}/products`);
                setProducts(productsRes.data);

                // Fetch Artist Services (Public)
                const servicesRes = await axios.get(`http://localhost:3001/api/users/artist/${id}/services`);
